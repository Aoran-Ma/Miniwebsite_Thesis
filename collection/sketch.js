...
let collectionImgs;
function preload() {
  let files = loadStrings("assets/file_list.txt", () => {
    for (let i = 0; i < files.length; i++) {
      imgs[i] = loadImage("assets/" + files[i]);
    }
  });
  collectionImgs = loadStrings("collection-imgs/file_list.txt");
  loadImage("collection-imgs/Ascencio-GayLiberator_04-01-1976_Page_13.png");
}
function setup() {
  ...
  let arr = shuffle(collectionImgs);
  for (let i = 0; i < arr.length; i++) {
    let img = createElement("img").class("item");
    img.elt.src = "collection-imgs/" + arr[i];
    select(".container").child(img);
  }
}
